...
async function tryAsyncCarpetas() {
  ...
  fs.writeFile("carpetasModelPreAwait.json", JSON.stringify(carpetasMap));

  for await (const carpeta of generateCarpetas()) {
    mapCarpetas.set(carpeta.numero, carpeta);

    await PrismaCarpeta.insertCarpeta(carpeta);
  }

  fs.writeFile("carpetasModelPostAwait.json", JSON.stringify(carpetasMap));
  return Array.from(mapCarpetas.values());
}

console.log(tryAsyncCarpetas());

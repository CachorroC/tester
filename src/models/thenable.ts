...
export class CarpetaJudicial implements IntCarpeta {
  ...
  constructor({
    ...
    this.demanda = new ClassDemanda(demanda, llaveProceso, this.procesos);
  ...
  async consultaProcesos() {
    ...
    try {
      ...
      json.procesos.forEach((proceso) => {
        if (!proceso.esPrivado) {
          activeProcesos.push(proceso);
        }
      });
      this.procesos = activeProcesos;
      this.idProcesos = json.procesos.map((prc) => {
        return prc.idProceso;
      });

      this.demanda.despachos = Array.from(despachosSet);
      this.demanda.sujetosProcesales = Array.from(sujetosProcesalesSet);

      this.demanda.juzgados = Array.from(juzgadosSet);
    ...
    try {
      ...
      for (const idProceso of this.idProcesos) {
        ...
        actuaciones.forEach((actuacion) => {
          actuacionesSet.add(actuacion);
        });

        actuacionesMap.set(idProceso, ultimaActuacion);

        const incomingDate = new Date(ultimaActuacion.fechaActuacion).getTime();

        const savedDate = this.fecha ? this.fecha.getTime() : null;

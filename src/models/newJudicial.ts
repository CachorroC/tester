...
export class NewJudicial implements IntCarpeta {
  ...
  notas: NotasBuilder[] = [];
  updatedAt: Date;
  constructor(rawCarpeta: RawDb, category: Category = "Bancolombia") {
    const {
      NUMERO: numero,
    ...
    if (OBSERVACIONES) {
      ...
      for (const extra of extras) {
        const newNoter = new NotasBuilder(extra, Number(numero));
    ...
    this.llaveProceso = String(llaveProceso);
    this.demanda = new ClassDemanda(rawCarpeta);
    this.nombre = String(DEMANDADO_NOMBRE).toLocaleLowerCase();
    this.revisado = false;
    this.codeudor = {
      ...
      for (const rawProceso of procesos) {
        ...
        const proceso = {
          ...
          fechaUltimaActuacion: rawProceso.fechaUltimaActuacion
            ...
            : null,
          juzgado: new NewJuzgado(rawProceso),
        };
  ...
  async updateProcesosinPrisma() {
    try {
      const prismaInserter = await client.proceso.createMany({
        data: this.procesos.map((proceso) => {
          const { juzgado, ...fixedProceso } = proceso;
          return {
            ...fixedProceso,
      ...
      try {
        ...
        if (
          ...
          this.ultimaActuacion = {
            ...ultimaActuacion,
            idProceso: idProceso,
            fechaActuacion: new Date(ultimaActuacion.fechaActuacion),
            fechaRegistro: new Date(ultimaActuacion.fechaRegistro),
            fechaFinal: ultimaActuacion.fechaFinal
              ? new Date(ultimaActuacion.fechaFinal)
              : null,
            fechaInicial: ultimaActuacion.fechaInicial
              ? new Date(ultimaActuacion.fechaInicial)
              : null,
            isUltimaAct:
        ...
        this.actuaciones = actuaciones.map((actuacion) => {
          return {
            ...actuacion,
            idProceso: idProceso,
            isUltimaAct:
  ...
  async updateActuacionesInPrisma() {
    try {
      const inserter = await client.actuacion.createMany({
        data: this.actuaciones.map((actuacion) => {
          return {
            ...actuacion,
            carpetaNumero: this.numero,
          };
    ...
    try {
      const upserter = await client.carpeta.upsert({
        ...
        create: {
          ...
          notas: {
            createMany: {
              data: this.notas.map((nota, index) => {
                console.log(index);
                return {
                  text: nota.text,
          ...
          deudor: {
            connectOrCreate: {
              ...
              create: {
                ...
                id: this.numero,
                cedula: String(this.deudor.cedula),
                telCelular: this.deudor.telCelular
                  ? String(this.deudor.telCelular)
                  : null,
                telFijo: this.deudor.telFijo
                  ? String(this.deudor.telFijo)
                  : null,
              },
            },
          },
...
async function getProcesosIdk() {
  ...
  for (const carpeta of Carpetas) {
    console.log(`inicia la carpeta numero ${carpeta.NUMERO}`);

    const newCarpeta = new NewJudicial(carpeta, carpeta.category);
    console.log(`${carpeta.NUMERO} inicia getProcesos`);
    await newCarpeta.getProcesos();
    console.log(`${carpeta.NUMERO} finaliza getProcesos`);

    console.log(`${carpeta.NUMERO} inicia getActuaciones`);
    await newCarpeta.getActuaciones();
    console.log(`${carpeta.NUMERO} finaliza getActuaciones`);

    console.log(`${carpeta.NUMERO} inicia prismaUpdater`);
    await newCarpeta.prismaUpdater();
    console.log(`${carpeta.NUMERO} finaliza prismaUpdater`);
    console.log(`${carpeta.NUMERO} inicia updateProcesosInPrisma`);
    await newCarpeta.updateProcesosinPrisma();
    console.log(`${carpeta.NUMERO} finaliza updateProcesosInPrisma`);
    console.log(`${carpeta.NUMERO} inicia updateActuacionesInPrisma`);
    await newCarpeta.updateActuacionesInPrisma();
    console.log(`${carpeta.NUMERO} finaliza updateActuacionesInPrisma`);

    newCarpetas.set(newCarpeta.numero, newCarpeta);
    fs.mkdir(`carpetas/${newCarpeta.numero}`, {
      recursive: true,
    });
    fs.writeFile(
      `carpetas/${newCarpeta.numero}/Carpeta.json`,
      JSON.stringify(newCarpeta, null, 2),
    );
  }

  console.log(JSON.stringify(newCarpetas, null, 2));
  /*
  try {
 ...
 */
  fs.writeFile(
    "Carpetas.json",
    JSON.stringify(Array.from(newCarpetas.values()), null, 2),
  );
}

getProcesosIdk().then((rr) => {
  console.log(rr);
  return rr;
});

...
export class ActuacionJudicial {
  ...
  constructor(idProceso: number, carpetaNumero: number) {
  ...
  async getActuaciones() {
    try {
      ...
      const savedYear = this._fecha ? this._fecha.getFullYear() : 2015;

      const savedMonth = this._fecha ? this._fecha.getMonth() : 0;

      const savedDay = this._fecha ? this._fecha.getDate() : 1;
      console.log(
        `${
          this.carpetaNumero
      ...
      if (
        ...
        this._fecha = new Date(ultimaActuacion.fechaActuacion);
      ...
      this._actuaciones = actuaciones.map((actuacion) => {
        return {
          ...actuacion,
          idProceso: this.idProceso,
          isUltimaAct:

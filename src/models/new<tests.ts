...
export class Carpeta implements IntCarpeta {
  ...
  constructor({ numero, deudor, category, codeudor, demanda }: CarpetaRaw) {
    this.numero = numero;
    this.nombre = deudor.nombre;
    this.category = category;
    this.revisado = category === "Terminados" ? true : false;
  ...
  idProcesos?: number[];
  nombre: string;
  set _llaveProceso(expediente: string) {
  ...
  async getProcesos() {
    try {
      ...
      this.idProcesos = procesos.map((prc) => {
        return prc.idProceso;
      });
      this.juzgados = procesos.map((proceso) => {
  ...
  async getActuaciones() {
    try {
      if (!this.procesos || this.procesos.length === 0) {
        throw new Error("no hay idProcesos en esta carpeta");
      }
      ...
      this.actuaciones = Array.from(actuacionesMap);
  ...
  async createPrismaCarpeta() {
    try {
      ...
      const prismaCarpetaPlain: Prisma.CarpetaCreateInput = {
        ...
        revisado: this.category === "Terminados" ? true : false,
      ...
      if (!actuacionesBuilder || actuacionesBuilder.length === 0) {
        return await client.carpeta.upsert({
          ...
          create: {
            ...prismaCarpetaPlain,
            idProcesos: procesosBuilder.map((proceso) => {
              return proceso.idProceso;
            }),
            procesos: {
              connectOrCreate: procesosBuilder.map((proceso) => {
                return {
                  where: {
                    idProceso: proceso.idProceso,
                  },
      ...
      const defUltimaAct = actuacionesBuilder.find((actuacion) => {
        return actuacion.isUltimaAct;
      });

      try {
        await client.actuacion.createMany({
          data: actuacionesBuilder.map((actuacion) => {
            return {
              ...actuacion,
              fechaActuacion: new Date(actuacion.fechaActuacion),
              fechaRegistro: new Date(actuacion.fechaRegistro),
          ...
          create: {
            ...
            ultimaActuacion: {
              connectOrCreate: {
                ...
                create: {
                  ...defUltimaAct,
                  fechaActuacion: new Date(defUltimaAct.fechaActuacion),
                  fechaRegistro: new Date(defUltimaAct.fechaRegistro),
                  fechaFinal: defUltimaAct.fechaFinal
                    ? new Date(defUltimaAct.fechaFinal)
                    : null,
                  fechaInicial: defUltimaAct.fechaInicial
                    ? new Date(defUltimaAct.fechaInicial)
                    : null,
            ...
            fecha: new Date(defUltimaAct.fechaActuacion),
          },
          update: {
            idProcesos: procesosBuilder.map((proceso) => {
              return proceso.idProceso;
  ...
  async updatePrismaCarpetawithProcesos() {
    try {
      if (!this.procesos || this.procesos.length === 0) {
        throw new Error("no hay procesos en esta carpeta");
      }
      ...
      for (const proceso of this.procesos) {
        ...
        const newJuzgado = new NewJuzgado(proceso);
        juzgadosSet.add(newJuzgado);
      }

      this.juzgados = Array.from(juzgadosSet);

      const juzgadosInCarpeta: Prisma.JuzgadoCreateOrConnectWithoutCarpetasInput[] =
        this.juzgados.map((juzgado) => {
          return {
            where: {
              tipo: juzgado.tipo,
            },
      ...
      return await client.carpeta.upsert({
        ...
        create: {
          ...
          idProcesos: this.procesos.map((prc) => {
            return prc.idProceso;
          }),
          revisado: this.category === "Terminados" ? true : false,
          terminado: this.category === "Terminados" ? true : false,

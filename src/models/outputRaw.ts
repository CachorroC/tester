...
async function mapperCarpetasToConsultaRaw() {
  ...
  for (const carpeta of Carpetas) {
    const { llaveProceso } = carpeta.demanda;

    try {
      const request = await fetcher(llaveProceso);
...
async function fetcher(llaveProceso: string) {
  try {
...
mapperCarpetasToConsultaRaw().then((ff) => {
  fs.writeFile("consultaAllFetch.json", JSON.stringify(ff, null, 2));
  console.log(ff);
  return ff;
});

...
function fechaPresentacionBuilder(
  ...
  const rawFechaPresentacionLength =
    rawFechaPresentacion.length;
  ...
  const [firstFecha, secondFecha, ...restFechas] =
...
function fixSingleFecha(rawFecha: string) {
  const [rawDay, rawMonth, rawYear] = rawFecha
    .trim()
    .split('/');
  console.log(rawDay);

...
import {
  ...
  insertProcesosInPrisma,
} from "./src/data/insert-carpetas";
import { PrismaCarpeta } from "./src/models/prisma-carpeta";

const date = new Date();
console.log(date.setSeconds(date.getSeconds() + 70));
...
async function f() {
  ...
  for (const rawCarpeta of sortedCarpetas) {
    ...
    const thener = new CarpetaJudicial(carpeta);
    console.log(thener.cc);

    const prismaCarpeta = new PrismaCarpeta(thener);
...
async function insertManyCarpetas() {
  ...
  for (const carpeta of carpetas) {
    ...
    console.log(
      `${numero} => mongoInsert: ${JSON.stringify(insertOne, null, 2)}`,
    );
    /*
    ...
    const prismaJuzgadoInserter = await insertJuzgadoInPrisma(carpeta);
    console.log(
    ...
    fs.writeFile(
      ...
      JSON.stringify(prismaJuzgadoInserter),
    );
    /*
    ...
    fs.writeFile(
      ...
      JSON.stringify(prismaProcesoInserter, (key, value) => {
        return typeof value === "bigint" ? value.toString() + "n" : value;
      }),
    );
    ...
    if (carpeta.ultimaActuacion) {
      ...
      fs.writeFile(
        ...
        JSON.stringify(prismaActuacionInserter, (key, value) => {
          return typeof value === "bigint" ? value.toString() + "n" : value;
        }),
      );
    }

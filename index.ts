...
async function f() {
  ...
  for (const rawCarpeta of sortedCarpetas) {
    ...
    fs.writeFile(
      ...
      JSON.stringify(idkFileWithProcesos, null, 2),
    );
    ...
    if (!thener.procesos || thener.procesos.length === 0) {
      ...
      for (const key in thener) {
        if (Object.prototype.hasOwnProperty.call(thener, key)) {
...
async function insertManyCarpetas() {
  ...
  for (const carpeta of carpetas) {
    try {
      const prismaCarpetaInserter = await insertCarpetaInPrisma(carpeta);
      fs.writeFile(
